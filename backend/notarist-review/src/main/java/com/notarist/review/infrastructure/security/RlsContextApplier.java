package com.notarist.review.infrastructure.security;
...
/**
 * Establishes the row-level-security identity for the current principal before an OCR-review query
 * runs.
 *
 * <p>A copy of the Case/Document module's applier rather than a shared one, on purpose: module
 * isolation keeps notarist-review from importing notarist-auth or notarist-document. Backed by the
 * same PostgreSQL RLS machinery (Flyway V9's {@code notarist_set_identity()}), which the review policy
 * in V12 reads back through {@code current_setting()}.
 *
 * <p>No system-identity escape. Reviews are always reached with a real authenticated principal.
 * Fail-closed: a session with no identity sees no reviews. Callers must be {@code @Transactional}; the
 * settings are transaction-local, so outside a transaction the write would not survive to the query.
 */
@Component("reviewRlsContextApplier")
